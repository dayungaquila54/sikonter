...
# Products Data
$purchases = $db->getConnection()->prepare("SELECT purchases.*, products.name FROM purchases LEFT JOIN products ON purchases.product = products.id WHERE purchases.users = ?");
$purchases->bind_param('s', $user['id']);
$purchases->execute();
$purchasesList = $purchases->get_result();
    ...
    <div class="container">
      ...
      <div class="collapse navbar-collapse" id="navbarBottom">
        <ul class="navbar-nav me-auto mb-2 mb-lg-0">
          ...
          <li class="nav-item">
            <a class="nav-link active" href="<?=base_url('purchases') ?>">Purchase</a>
          </li>
          <li class="nav-item">
      ...
      <div class="row">
        <div class="col-md-3">
          <div class="list-group">
            ...
            <a class="list-group-item active" href="<?=base_url('/purchase') ?>">Purchase</a>
            <a class="list-group-item" href="<?=base_url('/invoices') ?>">Invoice</a>
          </div>
        </div>
        <div class="col-md-9">
          <h1>Purchased Order</h1>
          <p>List of your purchased order.</p>
          ...
          <div class="table-responsive">
            <table class="table-striped table">
              ...
              <tbody>
                ...
                <?php while($data = $purchasesList->fetch_assoc()) : ?>
                  <?php
                  
                  $invoice = $db->getConnection()->prepare("SELECT * FROM invoices WHERE purchase_id = {$data['id']}");
                  $invoice->execute();
                  $invoiceData = $invoice->get_result()->fetch_assoc();
                  ?>
                <tr>
                  <td><?=$data['id'] ?></td>
                  <td><?=$data['name'] ?></td>
                  ...
                  <td>
                    <a href="<?=base_url("pay/{$invoiceData['id']}") ?>" class="btn btn-primary<?php if($data['status']) : ?> disabled<?php endif; ?>">Pay Now</a>
                  </td>
                </tr>
                <?php endwhile; ?>
                <?php else: ?>
